...
const NAMED = {
  ...
  /** The three names the switch governs, and the note, which has a switch of its own. */
...
test('turning the names off keeps all three out of the tags that would be written', () => {
  for (const kind of ['image', 'video']) {
    ...
    assert.ok(off.includes(NAMED.note), `${kind}: the note has its own switch`);
  ...
  try {
    for (const kind of ['image', 'video']) {
      ...
      for (const tagChildName of [false, true]) {
        ...
        // The note is a separate switch and stays on, which also shows the write happened
        // at all rather than the file simply being left as the mock served it.
        assert.ok(inFile(NAMED.note), `${kind}: the note has its own switch`);
      }
    }
  } finally {
    await reader.end();
