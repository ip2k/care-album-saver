...
/**
 * Show the archive in Finder, Explorer or the desktop's file manager.
 *
 * The path comes from the saved configuration, which `checkArchiveDir` has already passed,
 * and it is handed over as an argument rather than inside a command line — the same rule as
 * schedule.ts, for the same reason.
 */
export async function openFolder(dir: string): Promise<void> {
  const platform = osPlatform();
  const [file, args] =
    platform === 'darwin' ? ['open', [dir]] : platform === 'win32' ? ['explorer.exe', [dir]] : ['xdg-open', [dir]];
  await new Promise<void>((resolve, reject) => {
    execFile(file as string, args as string[], { windowsHide: true, timeout: 10_000 }, (error) => {
      // Explorer exits 1 even when it opened the window, so its status is not evidence of
      // anything; only a program that never started is worth reporting.
      if (error && (error as NodeJS.ErrnoException).code === 'ENOENT') {
        reject(new Error(`This computer has no "${file}" command, so the folder could not be opened for you. It is at ${dir}.`));
        return;
      }
      resolve();
    });
