...
| A malicious dependency | There are no third-party runtime dependencies: the one `dependencies` entry is `media-ferry`, a package in this repo with none of its own. ExifTool is an optional dependency and the tool degrades to JSON sidecars without it. The audit job installs with `--ignore-scripts` and runs `pnpm audit`; the test matrix installs normally, because it exercises the optional ExifTool. |
