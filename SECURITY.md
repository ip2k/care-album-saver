...
| Apple Photos.app, and so iCloud, given a file this tool did not save | Off unless turned on, and turned on only through the setup page, which asks macOS for permission first and asks the parent to confirm that Apple Photos.app's *Copy items to the Photos library* is on, since the tool cannot see that setting. Apple Photos.app is never handed the archive's own files: each is copied into a fresh owner-only hand-over folder in the config folder (an APFS clone where the disk allows), the copy is hashed and checked against `fingerprints.json` — the record of every file sync saved, kept in the config folder where something else that writes into the archive cannot edit it — and only a copy that matches is handed over; the rest are left out and named on the page. The AppleScript is a fixed file run by `/usr/bin/osascript` with arguments, never `-e`, and it refuses unless the Photos it would talk to, and every process running as Photos, is `/System/Applications/Photos.app`. When it works, the script prints one integer, how many items Apple Photos.app imported, and that is all that comes back: no names, paths or ids from the library. When it fails, the page shows the last line of the error osascript reports. `photos.json` records what has been handed over, by fingerprint, and is updated only after Apple Photos.app accepts a batch. No test can drive the real Apple Photos.app (`CARE_ALBUM_NO_PHOTOS`). See [docs/PHOTOS.md](docs/PHOTOS.md). |
